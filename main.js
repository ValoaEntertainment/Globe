...
function sideMenuConstructor(hasActiveTab, hasTabs) {
  let constructor = [{
    ...
    label: "Quit Globe", icon: app.getAppPath() + "/imgs/icons16/exit.png", accelerator: "CmdOrCtrl+Shift+Q", click: () => { 
      app.quit(); 
    } }
  ];
    ...
    loadTheme().then(({theme, dark}) => {
      ...
      loadWinControlsModule().then((winControls) => {
        mainWindow = new BrowserWindow({
          ...
          frame: winControls.systemTitlebar,
          // show: false,
          icon: app.getAppPath() + "/imgs/valoa.png",
          webPreferences: {
            nodeIntegration: true
          },
          backgroundColor: backgroundColor
